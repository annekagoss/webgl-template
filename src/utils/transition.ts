...
export const easeInOutQuad = (time: number, initial: number, final: number, duration: number): number => {
	if ((time /= duration / 2) < 1) return (final / 2) * time * time + initial;
	return (-final / 2) * (--time * (time - 2) - 1) + initial;
};

export const easeInOutCubic = (time: number, initial: number, final: number, duration: number) => {
